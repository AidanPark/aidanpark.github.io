...
RTL = {"ar"}
BASE = "https://aidanpark.github.io/guide/"
...
<main>
  <div class="topbar">
    <p class="backlink"><a href="/">&larr; BoxBee</a></p>
...
(function () {{
  ...
  sel.addEventListener('change', function () {{
    ...
    location.href = v === 'en' ? '/guide/' : '/guide/' + v + '/';
  }});
{redirect}}})();
</script>
</body>
...
REDIRECT_JS = """  // 첫 방문 시 브라우저 언어에 맞는 번역 페이지로 이동 (언어를 직접 고르면 그 선택을 기억)
  ...
  if (saved === 'en') return;
  if (saved && LANGS[saved]) { location.replace('/guide/' + saved + '/'); return; }
  if (saved) return;
...
def build():
    ...
    for code in ORDER:
        ...
        page = TEMPLATE.format(
            ...
            redirect=REDIRECT_JS if code == "en" else "",
            **t,
        )
